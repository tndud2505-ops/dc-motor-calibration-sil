...
{
    if (controller_state == STATE_CALIBRATE_TO_UPPER && current_raw >= STALL_CURRENT_RAW)
    {
        Motor_Stop();
        controller_state = STATE_WAIT_CURRENT_LOW;
    }
    else if (controller_state == STATE_WAIT_CURRENT_LOW && current_raw < STALL_CURRENT_RAW)
    {
        calibration_stroke = 0;
        controller_state = STATE_CALIBRATE_TO_LOWER;
        Motor_CCW();
    }
    else if (controller_state == STATE_CALIBRATE_TO_LOWER && current_raw >= STALL_CURRENT_RAW)
    {
        lower_point = 0;
...
{
    current_raw = SIL_ReadCurrentRaw();
}
...
uint32_t Control_GetCurrentRaw(void) { return current_raw; }
