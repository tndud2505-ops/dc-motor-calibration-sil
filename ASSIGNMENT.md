...
학생이 수정할 파일은 `code/sil_environment.c` 하나뿐입니다. 이 파일은 컴파일 가능한 빈 함수와 `TODO`로 제공됩니다. 같은 `code/` 폴더의 나머지 파일은 제공 코드이므로 수정하지 않습니다.
...
2. [LOCAL_BUILD.md](LOCAL_BUILD.md)에 따라 빈 시작 코드를 로컬에서 빌드합니다. 이 단계에서는 컴파일 성공만 확인합니다.
3. [SIL_GUIDE.md](SIL_GUIDE.md)를 읽고 `code/sil_environment.c`의 역할을 이해합니다.
4. SIL 환경을 수정한 뒤 로컬에서 다시 빌드합니다.
5. 마지막에 [GIT_SUBMISSION.md](GIT_SUBMISSION.md)를 따라 GitHub 계정을 만들고 학생 브랜치에 Push합니다.

## `code/sil_environment.c`에서 구현할 내용
...
- `code/provided_control.c`: CAL, GET ON 50%, GET OFF 10%, STOP 동작을 결정합니다.
- `code/main.c`: 명령을 순서대로 실행하고 상태 변수만 출력합니다.
- `code/sil_api.h`: 제공 코드와 SIL 환경 사이의 함수 규약입니다.
...
초록색 체크만으로 CAL 기능이 완성됐다고 판단하지 않습니다. `code/main.c`가 출력한 `calibration_complete`, `lower_point`, `upper_point`, `current_position` 값을 함께 확인합니다.
